...
SELECT
    AVG(t.nb) AS moyenne_inscriptions_utilisateur
FROM
    (
        SELECT
            u.id,
...
SELECT
    ROUND(AVG(t.revenu_utilisateur), 2) AS revenu_moyen_par_utilisateur_inscrit
FROM
    (
        SELECT
            e.user_id,
            SUM(c.price) AS revenu_utilisateur
        FROM
            enrollments e
            JOIN courses c ON c.id = e.course_id
        GROUP BY
